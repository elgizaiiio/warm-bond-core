...
const NoiseRemoverPage = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      await supabase.functions.invoke("deduct-credits", {
        body: { user_id: userId, amount: COST, action_type: "noise_removal", description: "Audio Noise Removal" }
      });

      const reader = new FileReader();
      reader.onload = async () => {
        const { data, error } = await supabase.functions.invoke("generate-voice", {
          body: { model_id: "noise-remover", prompt: reader.result, type: "denoise", settings: {} },
        });
        if (error) throw error;
        if (data?.url) { setResultUrl(data.url); setStep("result"); toast.success("Noise removed!"); }
  ...
  const renderStep = () => {
    switch (step) {
      case "upload":
        return (
          <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 px-4">
            <h2 className="text-xl font-bold text-foreground mb-2">Remove Background Noise</h2>
            <p className="text-sm text-muted-foreground mb-6 text-center">Upload an audio file with background noise and AI will clean it up</p>
            <button onClick={() => fileInputRef.current?.click()} className="w-full max-w-sm py-16 rounded-2xl border-2 border-dashed border-border/40 flex flex-col items-center gap-3 text-muted-foreground hover:border-primary/50 hover:text-foreground transition-colors">
              ...
              <p className="text-sm font-medium">Upload Noisy Audio</p>
        ...
        return (
          <div className="flex flex-col items-center justify-center min-h-[60vh] px-6 gap-6">
            ...
            <div className="text-center">
              <h2 className="text-xl font-bold text-foreground mb-2">Ready to Clean</h2>
      ...
      case "loading":
        return <VoiceStarLoader text="Removing background noise..." />;
    ...
    <AppLayout onSelectConversation={() => {}} onNewChat={() => {}} activeConversationId={null}>
      <div className="h-full flex flex-col bg-background">
        <div className="sticky top-0 z-10 px-4 py-3 bg-background/80 backdrop-blur-xl flex items-center gap-3">
          ...
          <h1 className="text-base font-bold text-foreground">Noise Remover</h1>
        </div>

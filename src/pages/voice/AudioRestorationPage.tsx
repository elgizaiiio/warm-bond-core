  ...
  const handleGenerate = async () => {
    ...
    try {
      await supabase.functions.invoke("deduct-credits", {
        body: { user_id: userId, amount: COST, action_type: "audio_restoration", description: "Audio Restoration" },
      });
      const reader = new FileReader();
      reader.onload = async () => {
        const { data, error } = await supabase.functions.invoke("generate-voice", {
          body: { model_id: "noise-remover", prompt: reader.result, type: "denoise", settings: { restore: true } },
        });
        if (error) throw error;
        if (data?.url) { setResultUrl(data.url); setStep("result"); toast.success("Audio restored!"); }
      ...
      case "upload":
        return (
          <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 px-4">
            <h2 className="text-xl font-bold text-foreground mb-2">AI Audio Restoration</h2>
            <p className="text-sm text-muted-foreground mb-6 text-center">Restore old or damaged audio recordings with AI</p>
        ...
        return (
          <div className="flex flex-col items-center justify-center min-h-[60vh] px-6 gap-6">
            ...
            <div className="text-center">
              <h2 className="text-xl font-bold text-foreground mb-2">Ready to Restore</h2>
            ...
            <motion.button whileTap={{ scale: 0.97 }} onClick={handleGenerate} className="w-full max-w-xs py-4 rounded-2xl bg-gradient-to-r from-violet-500 to-blue-500 text-white font-bold text-sm">Restore Audio</motion.button>
            <button onClick={() => setStep("upload")} className="text-sm text-muted-foreground">Change file</button>
          </div>
        );
      case "loading": return <VoiceStarLoader text="Restoring audio quality..." />;
      case "result":
        return (
          <div className="px-4 pt-4">
            {resultUrl && <VoiceResultPlayer audioUrl={resultUrl} title="Restored Audio" onGenerateMore={() => navigate("/voice")} />}
          </div>
        );
    }
  };
    ...
    <AppLayout onSelectConversation={() => {}} onNewChat={() => {}} activeConversationId={null}>
      <div className="h-full flex flex-col bg-background">
        <div className="sticky top-0 z-10 px-4 py-3 bg-background/80 backdrop-blur-xl flex items-center gap-3">
          ...
          <h1 className="text-base font-bold text-foreground">Audio Restoration</h1>
        </div>
...
export default AudioRestorationPage;

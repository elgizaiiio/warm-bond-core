  ...
  const handleGenerate = async () => {
    ...
    try {
      await supabase.functions.invoke("deduct-credits", {
        body: { user_id: userId, amount: COST, action_type: "podcast_edit", description: "AI Podcast Editor" },
      });
      const reader = new FileReader();
      reader.onload = async () => {
        const { data, error } = await supabase.functions.invoke("generate-voice", {
          body: { model_id: "noise-remover", prompt: reader.result, type: "denoise", settings: { enhance: true } },
        });
        if (error) throw error;
        if (data?.url) { setResultUrl(data.url); setStep("result"); toast.success("Podcast cleaned!"); }
      ...
      case "upload":
        return (
          <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 px-4">
            <h2 className="text-xl font-bold text-foreground mb-2">AI Podcast Editor</h2>
            <p className="text-sm text-muted-foreground mb-6 text-center">Remove filler words, normalize audio, and clean up your podcast</p>
            <button onClick={() => fileInputRef.current?.click()} className="w-full max-w-sm py-16 rounded-2xl border-2 border-dashed border-border/40 flex flex-col items-center gap-3 text-muted-foreground hover:border-primary/50 hover:text-foreground transition-colors">
              ...
              <p className="text-sm font-medium">Upload Podcast Audio</p>
        ...
        return (
          <div className="flex flex-col items-center justify-center min-h-[60vh] px-6 gap-6">
            ...
            <div className="text-center">
              <h2 className="text-xl font-bold text-foreground mb-2">Ready to Edit</h2>
            ...
            <motion.button whileTap={{ scale: 0.97 }} onClick={handleGenerate} className="w-full max-w-xs py-4 rounded-2xl bg-gradient-to-r from-violet-500 to-blue-500 text-white font-bold text-sm">Clean Podcast</motion.button>
            <button onClick={() => setStep("upload")} className="text-sm text-muted-foreground">Change file</button>
          </div>
        );
      case "loading": return <VoiceStarLoader text="Editing your podcast..." />;
      case "result":
        return (
          <div className="px-4 pt-4">
            {resultUrl && <VoiceResultPlayer audioUrl={resultUrl} title="Edited Podcast" onGenerateMore={() => navigate("/voice")} />}
          </div>
        );
    }
  };
    ...
    <AppLayout onSelectConversation={() => {}} onNewChat={() => {}} activeConversationId={null}>
      <div className="h-full flex flex-col bg-background">
        <div className="sticky top-0 z-10 px-4 py-3 bg-background/80 backdrop-blur-xl flex items-center gap-3">
          ...
          <h1 className="text-base font-bold text-foreground">Podcast Editor</h1>
        </div>
...
export default PodcastEditorPage;
